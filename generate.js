...
async function runRacePipeline(url, config) {
    ...
    function applyPartisanSponsorDiscount(poll) {
        ...
        return poll.responses.map(r =>
            r.party === sp ? { ...r, pct: (r.pct || 0) * 0.95 } : r
        );
...
const GOV_EXCLUDE_RE = /undecided|don't know|bishop|demings|dixon|porter|begich|swanson|steyer|lytle|duggan|stefanik|pizzo|bell|other|refused|would not vote/i;
